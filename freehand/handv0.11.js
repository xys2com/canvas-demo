...
class HandDraw {
	constructor(options) {
		...
		this.defLineColor = lineColor || "#000";
		this.defFillColor = fillColor || "#fff";
		this.TOTALINDEX = 0;
		this.canvasList = {
			[this.id]: this.canvas
		};
	...
	// 绘制一个矩形
	createRect(id, x, y, w, h, offset = 2, color, double = true) {
		const dot1 = { x, y },
			dot2 = { x: x + w, y },
			dot3 = { x: x + w, y: y + h },
			dot4 = { x, y: y + h };
		...
		graph.ctx = this.ctx;
		this.initLinePath(path, offset, color, "polygon", double, id);
		return graph;
	}
	// 绘制一个多边形
	createPolygon(dots, offset = 2, color) {}
	clear() {
	...
	scaleCommon(
		...
		this.dotin(referPoint, "#00f");
		this.dotin(targetPoint, "#0f0");
	...
	updateGraphZoom(graph, rate, rp) {
		...
		const cx = rp ? rp.x : x + width / 2;
		const cy = rp ? rp.y : y + height / 2;
		let _rp = {
			x: cx,
			y: cy
		};
	...
	refreshGraph(graph, color, log = false) {
		graph.lines.map((line) => {
			this.refreshLine(line, color, log);
		});
	}
	// 重绘所有图形
	// id 不刷新某个图形的id
	...
	initGraphStatus(graph, unhands = []) {
		...
		// graph.onenter = false; // 鼠标进入
		// graph.onpitch = false; //鼠标选中
		// graph.onmove = false; // 移动
		// graph.onzoom = false; // 缩放
	}
	// 重置所有活动中图形
	initAllGraphStatus() {
