...
function createMeteor(angle, ratio) {
  ...
  let aClr = ctx2.createRadialGradient(aCtr, aCtr, 0, aCtr, aCtr, aCtr);
...
let undulate = function (options, _img) {
  let settings = {
    speed: options["speed"] || 1,
    scale: options["scale"] || 1,
    waves: options["waves"] || 15,
    image: options["image"] || true,
    height: options["height"] || 1 / 2,
  };
  let waves = settings["waves"];
  let speed = settings["speed"] / 4;
  ...
  img.onload = function () {
    ...
    let id = c.getImageData(0, h * height, w, h).data;
    let end = false;
    c.save();
    // 关键内容
    while (!end) {
      let odd = c.getImageData(0, h * height, w, h);
      let od = odd.data;
      let pixel = 0;
      for (let y = 0; y < dh; y++) {
        for (let x = 0; x < dw; x++) {
          // 设置便宜量越靠近垂直中心，便宜量越小，
          // 此处可以想象一下， 屏幕中心发出一道正弦波，波越来越大
          // 根据此时y距离中心点的距离计算横向的像素点 x 轴的正负偏移量
          let displacement =
            (scale * 10 * Math.sin(dh / (y / waves) + -offset)) | 0;
          let j = ((displacement + y) * w + x + displacement) * 4;
          if (j < 0) {
            pixel += 4;
            continue;
          }
          let m = j % (w * 4);
          let n = scale * 10 * (y / waves);
          if (m < n || m > w * 4 - n) {
            let sign = y < w / 2 ? 1 : -1;
  ...
  // 使用requestAnimationFrame时降帧
  let fps = 30; // 1s 30fps
  let fpsInterval = 1000 / fps;
  let last = new Date().getTime();
  function anmfn() {
    window.requestAnimationFrame(anmfn);
    // 执行时的时间
