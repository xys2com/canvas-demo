...
const LINE = {
  ...
  getDelta(x1, x2, min, max) {
    ...
    if (rat < min || rat > max) {
      return this.getDelta(x1, x2, min, max);
    }
    return x1 + dx;
  },
  // 绘制一条二阶贝塞尔曲线
  ...
  calculateLine(x1, y1, x2, y2, offset, color, context) {
    ...
    // x轴中随机点位
    const _x2 = this.getDelta(_x1, _x4, slopeOffsetVal / 2, slopeOffsetVal); //_x1到_x4， 比率在 .125 - .25 之间的随机值
    ...
    const _y2 = this.getDelta(_y1, _y4, slopeOffsetVal / 2, slopeOffsetVal);

    const _x3 = this.getDelta(_x2, _x4, slopeOffsetVal2 / 2, slopeOffsetVal2); //_x2到_x4，比率在 0.1666 - 0.333 之间的随机值
    ...
    const _y3 = this.getDelta(_y2, _y4, slopeOffsetVal2 / 2, slopeOffsetVal2);
  ...
  initBzr3Line(options) {
    const { path, double = true, color, context } = options;
    let linepath = [];
    const offsetX = (path[6] - path[0]) / 20;
    const offsetY = (path[7] - path[1]) / 20;
    let line1 = [];
    for (let i = 0; i < path.length; i++) {
      if (i % 2 == 0) {
        line1.push(path[i] + offsetX * random(-1, 1));
      } else {
        line1.push(path[i] + offsetY * random(-1, 1));
      }
    }
    linepath.push(line1);
    this.bezierLineStage3(line1, color, context);
    if (double) {
      let line2 = [];
      for (let i = 0; i < path.length; i++) {
        if (i % 2 == 0) {
          line2.push(path[i] + offsetX * random(-1, 1));
        } else {
          line2.push(path[i] + offsetY * random(-1, 1));
        }
      }
      linepath.push(line2);
      this.bezierLineStage3(line2, color, context);
  ...
  // 重绘某个曲线
  refreshBzrLine(line, color, context, log = false) {
    const { zoom, zoomPath, path } = line;
    const paths = zoom !== 1 && zoomPath && zoomPath.length ? zoomPath : path;
  ...
  // 重绘所有曲线
  // id 不刷新的某个线条
  refreshAllBzrLine(id) {
    // 重绘线段
    for (let k in this.bzrLines) {
      const line = this.bzrLines[k];
      if (line.id != id && !line.staticstate) this.refreshBzrLine(line);
    }
  },
...
const GRAPH = {
  ...
  refreshAllGraph(id) {
    ...
    for (let gkey in gs) {
      const g = gs[gkey];
      console.log(id != g.id && !g.staticstate);
...
const CIRCLE = {
  ...
  createCircle(options, context, autodraw = true) {
    ...
    graph.double = options.hasOwnProperty("double") ? double : true;
    const offsetPX = r / 20;
    let _x = x + random(-offsetPX, offsetPX);
    let _y = y + random(-offsetPX, offsetPX);
    const points = this.calculateCirDots(_x, _y, r);
    graph.points = [];
    graph.points.push(...points);
    let len = 8;
    if (graph.double) {
      let __x = x + random(-offsetPX, offsetPX);
      let __y = y + random(-offsetPX, offsetPX);
  ...
  calculateCirDots(x, y, r) {
    ...
    for (let i = 0; i < len; i++) {
      ...
      let dx = random(-offsetPX, offsetPX);
      let dy = random(-offsetPX, offsetPX);
      let x = d.x + dx;
      let y = d.y + dy;
      dots.push({ x, y });
    }
