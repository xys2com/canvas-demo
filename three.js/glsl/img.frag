...
void main( ) {
	...
	float freqs[4];
	//Sound
	freqs[0] = noise(vec3( 0.01*100.0, 0.25 ,u_time/10.0) );
	freqs[1] = noise(vec3( 0.07*100.0, 0.25 ,u_time/10.0) );
	freqs[2] = noise(vec3( 0.15*100.0, 0.25 ,u_time/10.0) );
	freqs[3] = noise(vec3( 0.30*100.0, 0.25 ,u_time/10.0) );

	float t = field(p,freqs[2]);
	float v = (1. - exp((abs(uv.x) - 1.) * 6.)) * (1. - exp((abs(uv.y) - 1.) * 6.));
	
    //Second Layer
	vec3 p2 = vec3(uvs / (4.+sin(u_time*0.11)*0.2+0.2+sin(u_time*0.15)*0.3+0.4), 1.5) + vec3(2., -1.1, -1.);
	p2 += 0.25 * vec3(sin(u_time / 16.), sin(u_time / 12.),  sin(u_time / 128.));
	float t2 = mix(field2(p2,freqs[3]), field(p2, freqs[3]), .5);
