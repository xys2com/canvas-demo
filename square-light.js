(function () {
  ...
  function Square(
          ...
          this.fls.push(
            new FloatLight({
              ...
              pathData: {
                ...
                across,
                curveEy:
                  this.shadow.shdHeight > this.shadow2.shdHeight
                    ? this.shadow.shdHeight
                    : this.shadow2.shdHeight,
                curveEx: this.width,
              },
            })
          );
          ...
          let f = new FloatAny(
            {
              x:
                this.c.x -
                this.hf +
                ((this.hf * 2) / (floatAnyNum + 1)) * (i + 1),
              y: this.c.y - this.shadow.shdHeight / 2 + random(5, 20),
              w: e.width,
              h: e.height,
              fh: e.fh,
              obj: e.url ? e.url : null,
            },
            Ctx
          );
    ...
    this.compute = () => {
      ...
      if (!across) {
        ...
        let blockGrd = Ctx2.createRadialGradient(
          this.p.x + this.hf,
          this.p.y + this.hfh,
          0,
          this.p.x + this.hf,
        ...
        blockGrd.addColorStop(
          0.1,
          `rgba(${
            this.shadow.rgb.r + 80 > 255 ? 255 : this.shadow.rgb.r + 80
          }, ${this.shadow.rgb.g + 80 > 255 ? 255 : this.shadow.rgb.g + 80}, ${
            this.shadow.rgb.b + 80 > 255 ? 255 : this.shadow.rgb.b + 80
          }, 1)`
        );
        blockGrd.addColorStop(
          1,
          `rgba(${this.rgb.r}, ${this.rgb.g}, ${this.rgb.b}, ${this.ga})`
        );
        ...
        this.shadow.path = [
          ...
          [this.p.x + this.hf * 2, this.p.y],
          [
            this.p.x + this.hf * 2 + this.shadow.shdOffset,
            this.p.y - this.shadow.shdHeight,
          ],
        ...
        for (let i = 0; i < 5; i++) {
          grd.addColorStop(
            i * 0.2,
            `rgba(${this.shadow.rgb.r}, ${this.shadow.rgb.g}, ${
              this.shadow.rgb.b
            }, ${1 - i * 0.2 - 0.2})`
          );
        }
        ...
        this.shadow2.path = [
          ...
          [this.p.x + this.hf * 2, this.p.y],
          [
            this.p.x + this.hf * 2 + this.shadow2.shdOffset,
            this.p.y - this.shadow2.shdHeight,
          ],
          [
            this.p.x - this.shadow2.shdOffset,
            this.p.y - this.shadow2.shdHeight,
          ],
        ];
        this.shadow2.gradient = [
          this.p.x + this.hf,
          this.p.y + this.hfh,
          this.p.x + this.hf,
          this.p.y - this.shadow2.shdHeight,
        ...
        for (let i = 0; i < 5; i++) {
          grd2.addColorStop(
            i * 0.2,
            `rgba(${this.shadow2.rgb.r}, ${this.shadow2.rgb.g}, ${
              this.shadow2.rgb.b
            }, ${1 - i * 0.2 - 0.2})`
          );
        }
        ...
        blockGrd.addColorStop(
          0,
          `rgba(${this.shadow.rgb.r}, ${this.shadow.rgb.g}, ${this.shadow.rgb.b}, 1)`
        );
        blockGrd.addColorStop(
          1,
          `rgba(${this.rgb.r}, ${this.rgb.g}, ${this.rgb.b}, ${this.ga})`
        ...
        this.shadow.path = [
          ...
          [this.p.x, this.p.y + this.hf * 2],
          [
            this.p.x + this.shadow.shdHeight,
            this.p.y + this.hf * 2 + this.shadow.shdOffset,
          ],
        ...
        this.shadow2.path = [
          ...
          [this.p.x, this.p.y + this.hf * 2],
          [
            this.p.x + this.shadow2.shdHeight,
            this.p.y + this.hf * 2 + this.shadow2.shdOffset,
          ],
          [
            this.p.x + this.shadow2.shdHeight,
            this.p.y - this.shadow2.shdOffset,
          ],
        ];
        this.shadow2.gradient = [
          this.p.x,
    ...
    this.draw = () => {
      ...
      if (!this.shadow.isDraw) {
        this.shadow.twinkle
          ? (this.shadow.isDraw = false)
          : (this.shadow.isDraw = true);
      ...
      if (!this.shadow2.isDraw) {
        this.shadow2.twinkle
          ? (this.shadow2.isDraw = false)
          : (this.shadow2.isDraw = true);
  ...
  function InitPath({
    ...
    if (across) {
      ...
      path.push([
        p.x + (curveEy * 3) / 4 + random(-offsetX, offsetX),
        p.y - random(0, 2 * offsetY),
        p.x + (curveEy * 7) / 8 + random(-offsetX, offsetX),
        p.y - curveEx / 2 - random(0, 2 * offsetY),
      ]);
    } else {
      let dir = random(0, 1),
        nowY = y,
  ...
  // 浮光类
  function FloatLight({ rgb, trailing, flsSizeRound, trailingNum, pathData }) {
    ...
    this.move = () => {
      ...
      Ctx.shadowBlur = 10;
      Ctx.beginPath();
      let randomR = this.flsSizeRound ? random(15, 25) / this.sizeRate : 0.25;
      if (this.trailing) {
        this.ga = 1 - this.frame / this.count;
      }
      if (this.pathData.curve) {
        this.t > 0.5 ? (this.ga = 1 - this.t) : void 0;
      }
      let rdg = Ctx.createRadialGradient(
        this.nx,
        this.ny,
        0,
        this.nx,
  ...
  function shadowBall(ballShadowArr, rgb, trailingNum, r) {
    // 绘制拖尾
    ballShadowArr.forEach((e, i) => {
      Ctx.initCtx();
      let clr = `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, ${
        (i / trailingNum) * 0.8
      })`;
  ...
  // 创建浮动物体
  function FloatAny({ x, y, w, h, fh, obj }, ctx) {
    // s 接受一张图片 或者 canvas 对象
    this.x = x;
    this.y = y;
    this.w = w;
    this.h = h;
