...
function mixinHEXv2(c1, c2, rate) {
	const f = Math.floor;
	let c1_255 = hexFormat255(c1);
	let c2_255 = hexFormat255(c2);
	let r, g, b, a;
	let rate2 = 1 - rate;
	// 没有透明度
	r = prefixHex((f(c1_255.r * rate) + f(c2_255.r * rate2)).toString(16));
	g = prefixHex((f(c1_255.g * rate) + f(c2_255.g * rate2)).toString(16));
	b = prefixHex((f(c1_255.b * rate) + f(c2_255.b * rate2)).toString(16));

	let hex = `#${r}${g}${b}`;
	if (c1_255.a != 255 || c2_255.a != 255)
		hex += prefixHex((f(c1_255.a * rate) + f(c2_255.a * rate2)).toString(16));
	return hex;
}

//
function hexFormat255(c) {
	const s = c.split("#")[1];
	let rgb = {};
	if (s.length == 3 || s.length == 4) {
		...
		if (a) rgb.a = parseInt(a + a, 16);
	} else if (s.length == 6 || s.length == 8) {
		let arr = [];
		for (let i = 0; i <= s.length; i += 2) {
			let v = s.substring(i, i + 2);
			arr.push(v);
		}
		const [r, g, b, a] = arr;
		rgb = {
